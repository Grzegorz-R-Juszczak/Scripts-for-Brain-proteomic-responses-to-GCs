...
library(dplyr)
Paper_data2 <- distinct(Paper_data) # selecting unique rows
head(Paper_data, n = 8) # showing first n rows
Manual_search_IDs = dplyr::select (Paper_data2, c(Cured_Accession_no_Paper, NCBI_ID_manual_search, UniProt_ID_manual_search)) # selecting columns
...
UniProt_data <- UniProt_data [, c("From", "Entry", "Entry Name", "Protein names", "Gene Names (primary)", "GeneID", "RGD", "Organism", "Organism (ID)")] # setting column order
head(UniProt_data, n = 8) # showing first n rows

colnames(UniProt_data) <- c("From", "Entry_UniProt", "Entry_Name_UniProt", "Protein_names_UniProt", "Gene_Names_UniProt",  "NCBI_Gene_ID_UniProt", "RGD_ID_UniProt", "Organism_UniProt",  "Organism_ID_UniProt") # setting column names
head(UniProt_data, n=8) # showing first n rows
...
UniProt_data3 = dplyr::mutate(UniProt_data2, RGD_ID_UniProt_clean  = substr(RGD_ID_UniProt, 1, nchar(RGD_ID_UniProt) - 1)) # removing “;” located at the end of the IDs
...
#UniProt_data4_duplicatted = dplyr::filter(UniProt_data4_duplicatted, Organism_UniProt == "Rattus norvegicus (Rat)") # selection of rat identifiers
#head(UniProt_data4_duplicatted, n=10) # showing first n rows
...
####################################################################
####### Joining the manual search data and UniProt data ####################
####################################################################

Pap_Uni  = dplyr:: full_join(Manual_search_IDs_2, UniProt_data8, by = c("UniProt_ID_manual_search" = "From"), keep = FALSE, na_matches = "never") # joining the data
...
Pap_Uni$NCBI_ID_manual_search <-as.character(Pap_Uni$NCBI_ID_manual_search) # changing data type in selected column 
head(Pap_Uni, n=4) # showing first n rows

Pap_Uni_NCBI = dplyr:: left_join(Pap_Uni, ncbi_dataset2, by = c("NCBI_ID_manual_search" = "NCBI_GeneID"), keep = TRUE, na_matches = "never") # joining the data
...
ensembl = useEnsembl(biomart="genes", dataset="rnorvegicus_gene_ensembl") # selection of Ensembl biomart database and dataset
...
Ensembl = getBM(attributes=c('external_gene_name', 'description', 'ensembl_gene_id', 'entrezgene_id', 'rgd_id', 'gene_biotype'), mart = ensembl) # import of data from Ensembl 
head(Ensembl, n = 20) # showing first n rows
readr::write_csv(x = Ensembl, file = "Ensembl.csv") # data export

colnames(Ensembl) <- c("Ensembl_gene_symbol", "Ensembl_description", "Ensembl_gene_id", "NCBI_ID_Ensembl", "RGD_ID_Ensembl", "Ensembl_gene_type") # setting column names
...
Ensembl$RGD_ID_Ensembl <-as.character(Ensembl$RGD_ID_Ensembl) # changing data type in selected column 

##################################################################
####### Joining data #################################################
##################################################################

Pap_Uni_NCBI_Ensembl = dplyr:: left_join(Pap_Uni_NCBI, Ensembl, by = c("NCBI_ID_manual_search" = "NCBI_ID_Ensembl"), keep = TRUE, na_matches = "never") # joining the data
...
####################################################################
####### joining unmatched data based on RGD ID####
####################################################################
...
Pap_Uni_NCBI_Ensembl_V2 = dplyr:: left_join(Pap_Uni_NCBI_Ensembl_unmatched2, Ensembl, by = c("Nomenclature_ID_NCBI" = "RGD_ID_Ensembl"), keep = TRUE, na_matches = "never") # joining the data
...
head(Pap_Uni_NCBI_Ensembl_V4, n = 10) # showing first n rows
...
Pap_Uni_NCBI_Ensembl_V2_unmatched$Cured_Accession_no_Paper <-as.character(Pap_Uni_NCBI_Ensembl_V2_unmatched$Cured_Accession_no_Paper) # changing data type in selected column
head(Pap_Uni_NCBI_Ensembl_V2_unmatched, n = 8) # showing first n rows
...
Ambiguity_test = dplyr::select (Pap_Uni_NCBI_Ensembl_V5, c(Ensembl_gene_symbol, NCBI_ID_manual_search)) # selecting columns
head(Ambiguity_test, n=10) # showing first n rows

grouped <- tidyr::nest( dplyr::group_by(Ambiguity_test, `NCBI_ID_manual_search`) ) # grouping data according to the items in NCBI_ID_manual_search column
integrated <- purrr::map_dfr(
  .x = grouped$data,
  .f = function(data_for_given_probe) {     
    purrr::map_chr(
...
Ambiguity_test3 = dplyr::select (Ambiguity_test2, c(NCBI_ID_manual_search, Ambiguity_UniProtID_EnsemblSymbol)) # selecting columns
head(Ambiguity_test3, n=10) # showing first n rows

######################################################################
######## Joining the dataset with results of ambiguity test ######################
######################################################################

Pap_Uni_NCBI_Ensembl_V6 = dplyr:: full_join(Ambiguity_test3, Pap_Uni_NCBI_Ensembl_V5, by = c("NCBI_ID_manual_search" = "NCBI_ID_manual_search"), keep = FALSE, na_matches = "never") # joining the data
...
Paper_data2$Cured_Accession_no_Paper <-as.character(Paper_data2$Cured_Accession_no_Paper) # changing data type in selected column
head(Paper_data2, n = 8) # showing first n rows

Pap_Uni_NCBI_Ensembl_V7 = dplyr:: left_join(Paper_data2, Pap_Uni_NCBI_Ensembl_V6, by = c("Cured_Accession_no_Paper" = "Cured_Accession_no_Paper"), keep = FALSE, na_matches = "never") # joining the data
...
library(dplyr)
Pap_Uni_NCBI_Ensembl_V11 <- distinct(Pap_Uni_NCBI_Ensembl_V10)

readr::write_csv(x = Pap_Uni_NCBI_Ensembl_V11, file = "Pap_Uni_NCBI_Ensembl_final.csv") # data export
